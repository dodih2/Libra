...
<body>
	...
	<form id="form_kirim_pesan" method="post">
		...
		<?php 
			$query_penerima = mysqli_query($koneksi, "SELECT id_member, nama FROM user WHERE id_member != $id_member");
	...
	<table width="600" class="tabel_pesan" cellpadding="5" cellspacing="0">
		...
		<tbody>
<?php
	$query_daftar_pesan = mysqli_query($koneksi, "SELECT P.*, M.id_member, M.nama FROM pesan P, user M WHERE P.id_pengirim=M.id_member AND P.id_penerima='$id_member' ORDER BY P.id_pesan DESC");
